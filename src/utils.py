...
def char_counter(text: str) -> Dict[str, int]:
    ...
    """
    return {char: text.count(char) for char in text if char not in string.punctuation and char != " "}


def top_n_common(chars: Dict[str, int], n: int = 5) -> List[Tuple[str, int]]:
    """Возвращает n самых частых символов, используя sorted(..., key=lambda item: item[1], reverse=True)[:n]."""
